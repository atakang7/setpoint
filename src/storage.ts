...
export class RunStorage {
  ...
  constructor(baseDir: string, cwd = process.cwd(), id = createRunId()) {
    this.root = resolve(cwd, baseDir);
    this.id = id;
    this.runDir = join(this.root, "runs", id);
  }
  ...
  async writeRun(record: RunRecord): Promise<string> {
    const runPath = await this.writeJson("run.json", record);
  ...
  static async readLatest(baseDir: string, cwd = process.cwd()): Promise<RunRecord | null> {
    ...
    try {
      ...
      try {
        const ids = (await readdir(join(root, "runs"))).sort().reverse();
        if (!ids[0]) return null;
        return JSON.parse(
          await readFile(join(root, "runs", ids[0], "run.json"), "utf8"),
        ) as RunRecord;
      } catch {
        return null;
      }
